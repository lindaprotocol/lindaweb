...
import { LINDA_BIP39_PATH_INDEX_0 } from './address.js';

const INVALID_LINDA_PATH_ERROR_MSG = 'Invalid linda path provided';
...
export function generateRandom(password = '', path = LINDA_BIP39_PATH_INDEX_0, wordlist?: Wordlist) {
...
export function generateAccountWithMnemonic(
    mnemonic: string,
    path: string = LINDA_BIP39_PATH_INDEX_0,
    password: string | null | undefined = '',
    wordlist: Wordlist | null = null
) {
    // eslint-disable-next-line no-useless-escape
    if (!String(path).match(/^m\/44\'\/195\'/)) {
        throw new Error(INVALID_LINDA_PATH_ERROR_MSG);
    }
